...
template <typename TOp, comb::ICmpPredicate predicate>
class MinMaxConversionPattern : public HandshakeConversionPattern<TOp> {
public:
  using HandshakeConversionPattern<TOp>::HandshakeConversionPattern;
  void buildModule(TOp op, BackedgeBuilder &bb, RTLBuilder &s,
                   hw::HWModulePortAccessor &ports) const override {
    auto unwrappedIO = this->unwrapIO(s, bb, ports);
    this->buildUnitRateJoinLogic(s, unwrappedIO, [&](ValueRange inputs) {
      auto cmp = comb::ICmpOp::create(s.b, op.getLoc(), predicate, inputs[0],
                                      inputs[1]);
      // For min/max, equal-valued inputs are interchangeable.
      return s.mux(cmp, {inputs[1], inputs[0]});
    });
  };
};
...
static LogicalResult convertFuncOp(ESITypeConverter &typeConverter,
  ...
  patterns.insert<
      ...
      MinMaxConversionPattern<arith::MaxSIOp, comb::ICmpPredicate::sgt>,
      MinMaxConversionPattern<arith::MinSIOp, comb::ICmpPredicate::slt>,
      MinMaxConversionPattern<arith::MaxUIOp, comb::ICmpPredicate::ugt>,
      MinMaxConversionPattern<arith::MinUIOp, comb::ICmpPredicate::ult>,
      // HW operations.
      ...
      ExtModuleConversionPattern<math::AbsFOp>,
      ExtModuleConversionPattern<math::RoundEvenOp>,
      // Arith operations.
...
public:
  void runOnOperation() override {
    ...
    target.addLegalOp<UnrealizedConversionCastOp, hw::HWModuleOp,
                      hw::HWModuleExternOp, hw::OutputOp,
                      hw::InstanceOp>();
    target
