...
** TODO_NEXT End-to-End semantic equivalence of a PyTorch kernel to synthesizable RTL [10/10]
...
Demonstrate that the selected pipeline can lower a minimal PyTorch kernel to synthesizable RTL, integrate it into a complete FPGA top-level, generate a bitstream, and run a fixed test vector on real hardware with outputs matching the PyTorch reference (exact or tolerance-defined).
...
** TODO_NEXT Lowering of small LLM to RTL [10/10]
- State "TODO_NEXT"  from "DONE"       [2026-02-02 Mon 11:25]
...
Unintegrated RTL netlist of the model (RTLIL or Verilog/SystemVerilog)

Synthesis resource estimates produced by open-source tools

Bottleneck report identifying unsupported operators, failing passes, or performance pathologies in the toolchain
...
Operator inventory + failures (if any)



Reduced failing case if blocked


Run the pipeline through tiny stories 1M. Reduce failing test case.

With reduced test case, debug. Explore options of executing different passes, or reordering them. Also explore fixes to tools.

This is going to be a loop, until it passes every step, and we get .il. Don't know how to acknoledge that in the plan. Maybe by stages? Torch-MLIR, MLIR, CIRCT, yosys.

*** DONE Subtasks
CLOSED: [2026-01-29 Thu 20:24]
...
- State "TODO_NEXT"  from              [2026-01-28 Wed 15:38]
  Artifact: resource utilization report (Yosys stat and/or JSON stats) generated from the .il/design.

Bottleneck report: unsupported operators encountered, failing passes, required stubs/workarounds, and compile-time pathologies; includes links to minimized reproducers where applicable.
...
** TODO_NEXT FPGA integration and hardware validation [5/5]
- State "TODO_NEXT"  from "DONE"       [2026-02-02 Mon 11:25]
...
** TODO_NEXT Scaling and resource usage analysis [5/5]
- State "TODO_NEXT"  from "DONE"       [2026-02-02 Mon 11:25]
...
Success would be: extrapolated resource usage shows that a large model is within or near (<= 2x) the capacity of the target FPGA.

*** DONE Risks and mitigations
CLOSED: [2026-01-29 Thu 22:09]
...
*** DONE Subtasks
CLOSED: [2026-01-30 Fri 00:24]
- State "DONE"       from "TODO_NEXT"  [2026-01-30 Fri 00:24]
- State "TODO_NEXT"  from              [2026-01-27 Tue 12:43]
5.1: Research in which directions can tiny stories be scaled
5.2: Create nix derivations at different scaling points.
5.3: Graph scaling pattern, figuring out it its linear, geometrical, exponential etc.
*** DONE Deliverables
CLOSED: [2026-01-30 Fri 00:23]
- State "DONE"       from "TODO_NEXT"  [2026-01-30 Fri 00:23]
- State "TODO_NEXT"  from              [2026-01-27 Tue 12:43]
- Research in which directions can tiny stories be scaled
- Create nix derivations at different scaling points.
- Graph scaling pattern, figuring out it its linear, geometrical, exponential etc.
...
- strategies_conclusion.org, and flake.nix updated to use those that work
