...
- State "TODO"       from              [2026-02-16 Mon 22:17]
  ...
  reaches Yosys/slang.
  - =tiny-stories-1m-baseline-float-il= is still blocked by an explicit Yosys
    out-of-memory kill on this host rather than unresolved module failures.
  - =tiny-stories-1m-baseline-float-yosys-stat= now succeeds by emitting an
    explicit OOM bottleneck report with measured SV bundle and memory metrics:
    `467` files, `6,971,927` lines in =main.sv=, `433,040,128` total memory
    bits, and `411,705,344` bits in the four largest memories.
