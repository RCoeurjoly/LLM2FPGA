...
The succesful result is an unintegrated RTL netlist of the model (RTLIL or Verilog/SystemVerilog) and synthesis resource estimates, with any temporary operator stubs resolved in the final claimed flow.
...
- State "TODO"       from              [2026-02-16 Mon 22:17]
  ...
  Current downstream synthesis direction:
  the canonical Task 3 path is now the direct =tiny-stories-1m= mapped
  utilization flow:
  =tiny-stories-1m-sv -> tiny-stories-1m-il -> tiny-stories-1m-utilization-json -> tiny-stories-1m-utilization=
  This keeps the 3e claim on the emitted design itself and avoids selftest
  wrappers, external-memory blackboxing, and alternate utilization routes.
  ...
  is the only canonical 3e gate. At the moment it is blocked earlier in the
  mapped path at =tiny-stories-1m-hw0=: =lower-handshake-to-hw= rejects
  unsupported float legalization, first at
  =arith.uitofp= in =tiny-stories-1m-hs-ext.mlir=, so the direct mapped report
  still needs a fresh successful build before updated utilization numbers can
  be recorded here.

  Follow-on direction:
  keep the clean quantization path and Task 6 resource minimization work open,
  but do it against the direct canonical mapped path rather than a shell-only
  surrogate.
