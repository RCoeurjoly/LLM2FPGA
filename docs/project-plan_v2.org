...
- State "TODO"       from              [2026-02-16 Mon 22:17]
  ...
  the selftest JSON/utilization path now has an explicit shell mode that
  externalizes large =handshake_memory_out_f32_*= modules before the staged
  =synth_xilinx= run. This keeps the synthesis target focused on the shell and
  controller fabric while treating the very large model/state memories as
  external-memory candidates.

  Current implementation status:
  - =scripts/pipeline/externalize_large_memories.py= selects oversized memory
    modules from the RTLIL
  - =mkTinyStoriesSelftestBundle= now synthesizes a =modelShellIl= produced by
    blackboxing those modules
  - =scripts/dev/build-tiny-stories-selftest-shell-local.sh= mirrors the same
    staged flow for fast local iteration

  Current validated artifact:
  =nix build .#tiny-stories-1m-selftest-all-memory-utilization -L=
  now succeeds and records the shell-mode resource report together with the
  external-memory candidate set.

  Recorded shell utilization:
  ...
  the current successful 3e artifact is an external-memory shell report rather
  than a claim that the full model fits on-chip.
