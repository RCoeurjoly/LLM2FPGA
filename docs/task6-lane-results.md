...
| What is the current mainline execution order? | Freeze StreamTensor-lite references as the comparison harness; keep external memory as the mainline lane; keep the LSQ A/B recorded as spent negative; keep minimized representative-core PT2E-static `cf-stats` as the live quant reference surface; and use the rebased local CIRCT patch stack as the current toolchain unblock attempt, because it clears the old upstream `-flatten-memref` path through compile but the packaged derivation is now stopped one step later by a single `check-circt` failure in `Conversion/HandshakeToHW/test_buffer.mlir` | Deep research audits plus the 2026-04-24 external-memory rerun, direct crash reproducers, `flatten-memref` isolation, fork patch replay, and rebased fork patch replay | decided |
| Which quantized route stays active under the amended plan? | The surviving full-model reference route is still `tiny-stories-1m` PT2E-static, and the active bounded spike is now the minimized `tiny-stories-1m-representative-core-pt2e-static` replay surface; it survives to real `cf-stats`, the rebased local CIRCT patch stack gets far enough to rebuild CIRCT and enter regression tests, and the current blocker is now the shared patched-toolchain `check-circt` failure in `Conversion/HandshakeToHW/test_buffer.mlir`, not the original upstream `-flatten-memref` crash | Existing Task 6 quant notes plus the 2026-04-24 representative-core PT2E-static reruns, `flatten-memref` isolation, and rebased fork patch replay | decided |
