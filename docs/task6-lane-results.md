...
| What is the current mainline execution order? | Freeze StreamTensor-lite references; keep the bounded DDR3 pass recorded as partial-shell; keep PT2E-static active; treat the one bounded alternate-lowering pass as spent negative; make quantized extracted-op parity the next active architecture item before any new kernel design | Deep research audit plus amended lane plan plus the 2026-04-24 LSQ `L1` A/B | decided |
| Which quantized route stays active under the amended plan? | `tiny-stories-1m` PT2E-static only; keep `dynamic-int8` and `torchao` frozen unless importer behavior changes | Existing Task 6 quant notes plus the 2026-04-24 bounded replay | decided |
