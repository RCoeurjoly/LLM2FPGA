...
| 2026-04-23 | `just task6-l0` | synthetic external-weight `64x64` GEMV | stage-local runner replay | `4` | `0` | `32,449` | `46,736` | `3.17 s / 2.26 s / 3.14 s` | `561,868 KB / 438,160 KB / 563,264 KB` | pass-surface fail-lut | keep the `L0` runner as a quick kernel-plumbing replay only; it reproduces the frozen DSP/FF proof and the expected LUT miss |
| 2026-04-23 | `just task6-l1` | block-0 `mlp.c_fc` extracted from `tiny-stories-1m-representative-core-v64-h4` | stage-local runner replay | `4` | `0` | `29,778` | `46,352` | `3.13 s / 2.26 s / 3.16 s` | `564,012 KB / 438,320 KB / 563,272 KB` | pass-surface | keep `task6-l1-c-fc-redirect-index-ring3-postbranch-outbuf-fifo2-abc9` as the `L1` gold reference; the new `just` surface reproduces the passing scorecard directly |
| 2026-04-23 | `just task6-l2` | `tiny-stories-v1k-h64-l1` tiled `4 x 64` replay | stage-local runner replay | `4` | `0` | `31,907` | `45,932` | `3.11 s / 2.25 s / 3.17 s` | `563,904 KB / 437,824 KB / 562,932 KB` | pass-surface fail-lut | keep tiled `L2` as the active mainline but leave `L3` blocked; the runner reproduces the current `2,047` LUT gap without reopening monolithic surgery |
| 2026-04-23 | `just task6-l3` | planned `tiny-stories-v4k-h64-l1` | stage-local runner gate | n/a | n/a | n/a | n/a | n/a | n/a | blocked | preserve the promotion rule in the command surface itself: `L3` emits a blocked summary until `L2` clears the ceiling or a new structural hypothesis changes the lane |
| 2026-04-23 | `just task6-l4` / `task6-x1` / `task6-x2` / `task6-x3` | reserve and deferred ladder rungs | stage-local runner gate sweep | n/a | n/a | n/a | n/a | n/a | n/a | blocked | keep the whole ladder honest at the command surface too: reserve and deferred rungs emit explicit blocked summaries, and the collision-safe runner now allocates distinct run directories even when several blocked commands launch in the same second |
