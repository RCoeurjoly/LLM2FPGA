...
| What is the current mainline execution order? | Freeze StreamTensor-lite references as the comparison harness; keep external memory as the mainline lane, but record that both the baseline float shell and the minimized representative-core PT2E-static spike are now blocked by the same restored-`circt-nix` upstream crash in `-flatten-memref`; keep the LSQ A/B recorded as spent negative; keep minimized representative-core PT2E-static `cf-stats` as the live quant reference surface below that blocker; require a new quant/storage hypothesis before any new kernel design or heavier replay | Deep research audits plus the 2026-04-24 external-memory rerun, direct crash reproducers, `flatten-memref` isolation, and representative-core PT2E-static spike | decided |
| Which quantized route stays active under the amended plan? | The surviving full-model reference route is still `tiny-stories-1m` PT2E-static, and the active bounded spike is now the minimized `tiny-stories-1m-representative-core-pt2e-static` replay surface; it survives to real `cf-stats` with explicit `quantize_per_tensor` structure, but its post-`cf` lowering is blocked by the same upstream CIRCT `-flatten-memref` crash as the baseline float shell | Existing Task 6 quant notes plus the 2026-04-24 representative-core PT2E-static reruns and `flatten-memref` isolation | decided |
