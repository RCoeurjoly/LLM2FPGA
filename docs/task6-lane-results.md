...
| What is the current mainline execution order? | Freeze StreamTensor-lite references as the comparison harness; keep external memory as the mainline lane; keep the LSQ A/B recorded as spent negative; keep minimized representative-core PT2E-static `cf-stats` as the live quant reference surface; and use the rebased local CIRCT patch stack as the current toolchain base, because it now clears the old upstream `-flatten-memref` crash and passes CIRCT checks, leaving the external-memory float lane ready to rerun while the quantized handshake spike is blocked separately on a missing `lower-cf-to-handshake=lsq` option | Deep research audits plus the 2026-04-24 external-memory rerun, direct crash reproducers, `flatten-memref` isolation, fork patch replay, rebased fork patch replay, and rebased fork patch replay plus buffer-test fix | decided |
| Which quantized route stays active under the amended plan? | The surviving full-model reference route is still `tiny-stories-1m` PT2E-static, and the active bounded spike is now the minimized `tiny-stories-1m-representative-core-pt2e-static` replay surface; it survives to real `cf-stats`, the rebased local CIRCT patch stack now fully rebuilds CIRCT under Nix, and the current blocker has moved from CIRCT itself to the route-specific missing `lower-cf-to-handshake=lsq` option expected by the quantized handshake derivation | Existing Task 6 quant notes plus the 2026-04-24 representative-core PT2E-static reruns, `flatten-memref` isolation, rebased fork patch replay, and rebased fork patch replay plus buffer-test fix | decided |
...
| representative-core PT2E-static quant spike | `tiny-stories-1m-representative-core-pt2e-static-{torch,cf-stats,cf,handshake}` | experimental |
...
| 2026-04-24 | `task6-l0-gemv64-yosys-stat` upstream warm-up probe | synthetic `64x64` GEMV smoke | cheapest CIRCT-dependent Task 6 target | pending toolchain bootstrap | pending toolchain bootstrap | pending toolchain bootstrap | pending toolchain bootstrap | `32.17 s` before manual stop | no final target RSS; log reached `llvm-tblgen` configure completion | block-toolchain-reentry | even the smallest CIRCT-dependent Task 6 surface re-enters the same upstream bootstrap before any Task 6 IR stage, so the current blocker is global toolchain warm-up, not `top4-memory` specifically |
| 2026-04-24 | `tiny-stories-1m-baseline-float-selftest-top4-memory-utilization` after restoring `circt-nix` as shipped | full TinyStories baseline shell | narrowed external-memory shell | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `16 s` | `565,464 KB` | block-upstream-circt-flatten-memref-crash | the external-memory mainline is no longer blocked on bootstrap; it now reaches real TinyStories lowering and crashes in upstream `circt-opt` while building `tiny-stories-1m-baseline-float-handshake.mlir`, and the later direct pass isolation shows the concrete shared blocker is `-flatten-memref`, not handshake legalization as a whole |
| 2026-04-24 | direct `circt-opt` replay on `tiny-stories-1m-baseline-float-cf.mlir` | full TinyStories baseline shell | standalone `cf -> flatten-memref` reproducer | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `2.35 s` | `93,940 KB` | pass-shared-flatten-memref-reproducer | the upstream CIRCT crash is reproducible outside the Nix shell pipeline on the stored baseline `cf.mlir` with `-flatten-memref` alone, with no output produced and the stack centered on `DenseElementsAttr::getNumElements()`, so this is a clean blocker reference rather than a wrapper artifact |
...
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-cf-stats` first attempt | minimized representative-core full-model quant spike | PT2E-static quantized frontend through LSQ pipeline | pending evaluation | pending evaluation | pending evaluation | pending evaluation | `1.24 s` | `184,688 KB` | block-untracked-quant-surface | the bounded minimized quant spike is structurally wired but the first attempt fails before PyTorch export because the new adapter file is not yet tracked by Git and is therefore omitted from the flake source snapshot; track it and rerun the same gate unchanged |
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-cf-stats` rerun on tracked tree | minimized representative-core full-model quant spike | PT2E-static quantized frontend through LSQ pipeline | pending pre-handshake | pending pre-handshake | pending pre-handshake | pending pre-handshake | `49.58 s` | `293,732 KB` | pass-quant-minimized-cf | the minimized representative-core PT2E-static route is a real surviving quantized full-model surface in this branch: the exported `torch` MLIR contains `66` `quantize_per_tensor` ops around `17` matmuls, the route reaches real `cf-stats`, and the first bounded quant spike therefore survives on minimized TinyStories instead of collapsing immediately to the old extracted-op no-op |
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-handshake` | minimized representative-core full-model quant spike | PT2E-static quantized frontend through LSQ handshake lowering | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `5.45 s` | `421,944 KB` | block-shared-upstream-circt-flatten-memref-crash | the minimized quant spike and the external-memory baseline now share the same restored-`circt-nix` blocker: the later direct pass isolation shows upstream CIRCT crashes already in `-flatten-memref` with the same `DenseElementsAttr::getNumElements()` stack signature, so quantization should not be widened past `cf-stats` until that toolchain issue is removed or worked around |
| 2026-04-24 | direct `circt-opt` replay on `tiny-stories-1m-representative-core-pt2e-static-cf.mlir` | minimized representative-core full-model quant spike | standalone `cf -> flatten-memref` reproducer | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `1.29 s` | `43,764 KB` | pass-shared-flatten-memref-reproducer | the minimized representative-core PT2E-static quant spike reproduces the same upstream CIRCT crash on `-flatten-memref` alone, so the quant lane and the external-memory lane are blocked by the same one-pass toolchain failure rather than by different downstream handshake behaviors |
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-handshake` with local `/home/roland/circt` patch stack | minimized representative-core full-model quant spike | first replay of the April 20 fork fix series on top of `circt-nix` as shipped | pending build | pending build | pending build | pending build | `6.15 s` | `421,892 KB` | block-circt-patch-drift | the attempted fork replay gets past Nix evaluation and into CIRCT `patchPhase`, but the first functional `FlattenMemRefs` patch rejects on the newer upstream source (`Hunk #5 FAILED at 515`), so the immediate blocker becomes source drift rather than the runtime `flatten-memref` crash itself |
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-handshake` with rebased local `/home/roland/circt` patch stack | minimized representative-core full-model quant spike | rebased April 20 fork fixes on top of `circt-nix` as shipped | pending build | pending build | pending build | pending build | `803.01 s` | `421,760 KB` | block-circt-check-buffer-test | the rebased patch stack applies cleanly and carries CIRCT through full compile plus `check-circt`, which is strong evidence that the old upstream `flatten-memref` crash path is removed, but the packaged derivation still stops before the Task 6 `handshake` artifact because one CIRCT regression test now fails in `Conversion/HandshakeToHW/test_buffer.mlir` on updated buffer-lowering constants |
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-handshake` with rebased local `/home/roland/circt` patch stack plus buffer-test fix | minimized representative-core full-model quant spike | rebased April 20 fork fixes plus the matching `test_buffer.mlir` expectation update on top of `circt-nix` as shipped | pending later-stage score | pending later-stage score | pending later-stage score | pending later-stage score | `829.21 s` | `421,940 KB` | block-missing-lsq-option | the patched upstream stack now fully rebuilds CIRCT and passes the earlier buffer regression, which removes the shared `flatten-memref`/`check-circt` blocker class, but the quantized representative-core handshake route still fails because its pipeline asks for `lower-cf-to-handshake=lsq` and this toolchain does not provide that option |
