...
| What is the current mainline execution order? | Freeze StreamTensor-lite references as the comparison harness; keep external memory as the mainline lane, but record that the restored `circt-nix` upstream pair now crashes on `tiny-stories-1m-baseline-float-handshake` before the `top4-memory` shell can complete; keep the LSQ A/B recorded as spent negative; use one minimized representative-core PT2E-static quant replay as the active bounded spike; require a new quant/storage hypothesis before any new kernel design or heavier replay | Deep research audits plus the 2026-04-24 external-memory rerun after restoring `circt-nix` as shipped and the representative-core PT2E-static spike setup | decided |
| Which quantized route stays active under the amended plan? | The surviving full-model reference route is still `tiny-stories-1m` PT2E-static, but the active bounded spike is now the new minimized `tiny-stories-1m-representative-core-pt2e-static` replay surface; the direct external-weight `L1` extracted-op parity slice remains rejected because it collapses to the same float `aten.matmul` export | Existing Task 6 quant notes plus the 2026-04-24 representative-core PT2E-static spike setup | decided |
