...
| What is the current mainline execution order? | Freeze StreamTensor-lite references as the comparison harness; keep external memory as the mainline lane, but record that both the baseline float shell and the minimized representative-core PT2E-static spike are now blocked by the same restored-`circt-nix` upstream crash in `-handshake-legalize-memrefs`; keep the LSQ A/B recorded as spent negative; keep minimized representative-core PT2E-static `cf-stats` as the live quant reference surface below that blocker; require a new quant/storage hypothesis before any new kernel design or heavier replay | Deep research audits plus the 2026-04-24 external-memory rerun, direct crash reproducer, and representative-core PT2E-static spike | decided |
| Which quantized route stays active under the amended plan? | The surviving full-model reference route is still `tiny-stories-1m` PT2E-static, and the active bounded spike is now the minimized `tiny-stories-1m-representative-core-pt2e-static` replay surface; it survives to real `cf-stats` with explicit `quantize_per_tensor` structure, but its `handshake` gate is blocked by the same upstream CIRCT crash as the baseline float shell | Existing Task 6 quant notes plus the 2026-04-24 representative-core PT2E-static reruns | decided |
...
| 2026-04-24 | `tiny-stories-1m-baseline-float-selftest-top4-memory-utilization` after restoring `circt-nix` as shipped | full TinyStories baseline shell | narrowed external-memory shell | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `16 s` | `565,464 KB` | block-upstream-circt-handshake-crash | the external-memory mainline is no longer blocked on bootstrap; it now reaches real TinyStories lowering and crashes in upstream `circt-opt` during `-handshake-legalize-memrefs` while building `tiny-stories-1m-baseline-float-handshake.mlir`, so this is a concrete upstream CIRCT blocker rather than a shell-fit verdict |
| 2026-04-24 | direct `circt-opt` replay on `tiny-stories-1m-baseline-float-cf.mlir` | full TinyStories baseline shell | standalone `cf -> handshake-legalize-memrefs` reproducer | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `1.42 s` | `94,720 KB` | pass-reproducer | the upstream CIRCT crash is reproducible outside the Nix shell pipeline on the stored baseline `cf.mlir`, with no output produced and the stack centered on `DenseElementsAttr::getNumElements()`, so this is a clean blocker reference rather than a wrapper artifact |
...
| 2026-04-24 | `tiny-stories-1m-representative-core-pt2e-static-handshake` | minimized representative-core full-model quant spike | PT2E-static quantized frontend through LSQ handshake lowering | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | pending crash before mapped result | `5.45 s` | `421,944 KB` | block-shared-upstream-circt-handshake-crash | the minimized quant spike and the external-memory baseline now share the same restored-`circt-nix` blocker: upstream CIRCT crashes in `-handshake-legalize-memrefs` with the same `DenseElementsAttr::getNumElements()` stack signature, so quantization should not be widened past `cf-stats` until that toolchain issue is removed or worked around |
