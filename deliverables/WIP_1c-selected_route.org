...
- Only one work (*StreamTensor*) demonstrates a full PyTorch-to-FPGA pipeline for an LLM using MLIR-based tooling. However, its reference implementation is no longer publicly available, preventing direct reuse or reproduction.
...
- It is fully open-source and does not depend on proprietary HLS or vendor-specific HDL features.
...
- The approach has precedent in published work (*HLSfromPyTorch* and *StreamTensor*), demonstrating technical feasibility.
...
If the selected route becomes partially or fully unviable, the following fallbacks will be used:

1. *Operator decomposition / model simplification*  
   Unsupported operators will be rewritten into simpler MLIR primitives, or the model will be reduced in scope.

2. *ONNX-based lowering*  
   If Torch-MLIR proves unsuitable for structural reasons:  
   #+begin_example
   PyTorch → ONNX → ONNX-MLIR → CIRCT → Verilog
   #+end_example

3. *Minimal handwritten HDL kernel*  
   As a last resort, a minimal transformer-style kernel will be implemented directly in Verilog to ensure the project still delivers an open-source FPGA baseline.
