...
- All HLS-based candidates use proprietary tools such as Vivado HLS (Vitis HLS). Panda Bambu seems to be the most promissing open source alternative, but compatibility with Vitis HLS code, specially its headers and pragmas is limited, so manual rework is needed.
