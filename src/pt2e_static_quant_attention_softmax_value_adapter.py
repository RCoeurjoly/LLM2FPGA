...
"""PT2E static-quantized softmax(value-attention) reproducer for torch-mlir."""
...
class QuantAttentionSoftmaxValue(torch.nn.Module):
...
def export_program(model_path: str | None) -> torch.export.ExportedProgram:
    del model_path
    model = QuantAttentionSoftmaxValue().eval()
