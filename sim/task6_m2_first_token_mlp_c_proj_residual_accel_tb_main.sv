...
module task6_m2_first_token_mlp_c_proj_residual_accel_tb;
  `include "tb_data.sv"
  ...
  logic [31:0] c_proj_sample0_o;
  logic [31:0] c_proj_sample1_o;
  logic [511:0] c_proj_vector_o;
  logic [31:0] final_checksum_o;
  logic [31:0] final_sample0_o;
  logic [31:0] final_sample1_o;
  logic [511:0] final_vector_o;
  logic [31:0] debug_o;
  logic [511:0] expected_c_proj_vector;
  logic [511:0] expected_final_vector;
  integer cycles;
  integer i;

  task6_m2_first_token_mlp_c_proj_residual_accel_top dut (
    .SYS_CLK(SYS_CLK),
    .SYS_RSTN(SYS_RSTN),
    .start_i(start_i),
    .status_o(status_o),
    ...
    .c_proj_sample0_o(c_proj_sample0_o),
    .c_proj_sample1_o(c_proj_sample1_o),
    .c_proj_vector_o(c_proj_vector_o),
  ...
  initial begin
    ...
    SYS_RSTN = 1'b0;
    start_i = 1'b0;
    expected_c_proj_vector = 512'd0;
    expected_final_vector = 512'd0;
    cycles = 0;

    for (i = 0; i < MLP_C_PROJ_OUT_DIM; i = i + 1) begin
      expected_c_proj_vector[i * 8 +: 8] = mlp_c_proj_expected_q[i];
    ...
    while (cycles < TIMEOUT_CYCLES) begin
      ...
      if (status_o[2]) begin
        $fatal(
          1,
          "FAIL: task6 M2 first-token MLP c_proj residual error status=%08x debug=%08x",
          status_o,
          debug_o
        );
      end
      ...
      if (status_o[6:4] == ST_DONE && status_o[3]) begin
        ...
        if (c_proj_sample0_o !== MLP_C_PROJ_EXPECTED_SAMPLE0) begin
          $fatal(1, "FAIL: c_proj sample0 expected %08x got %08x", MLP_C_PROJ_EXPECTED_SAMPLE0, c_proj_sample0_o);
        end
        if (c_proj_sample1_o !== MLP_C_PROJ_EXPECTED_SAMPLE1) begin
          $fatal(1, "FAIL: c_proj sample1 expected %08x got %08x", MLP_C_PROJ_EXPECTED_SAMPLE1, c_proj_sample1_o);
        end
        if (c_proj_vector_o !== expected_c_proj_vector) begin
          $fatal(1, "FAIL: c_proj vector mismatch");
        end
        ...
        $display(
          "PASS: task6 M2 first-token MLP c_proj residual cycles %0d c_proj_checksum %08x c_proj_sample0 %08x c_proj_sample1 %08x final_checksum %08x final_sample0 %08x final_sample1 %08x",
          cycle_count_o,
          c_proj_checksum_o,
          c_proj_sample0_o,
          c_proj_sample1_o,
    ...
    $fatal(1, "Timeout waiting for task6 M2 first-token MLP c_proj residual done");
  end
endmodule
