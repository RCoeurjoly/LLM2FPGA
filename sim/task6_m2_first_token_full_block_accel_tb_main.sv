...
module task6_m2_first_token_full_block_accel_tb;
  `include "tb_data.sv"
  ...
  task6_m2_first_token_full_block_accel_top dut (
    .SYS_CLK(SYS_CLK),
    .SYS_RSTN(SYS_RSTN),
    .start_i(start_i),
    .status_o(status_o),
  ...
  initial begin
    ...
    while (cycles < TIMEOUT_CYCLES) begin
      ...
      if (status_o[2]) begin
        $fatal(
          1,
          "FAIL: task6 M2 first-token full-block error status=%08x debug=%08x",
          status_o,
          debug_o
        );
      end
      ...
      if (status_o[6:4] == ST_DONE && status_o[3]) begin
        ...
        $display(
          "PASS: task6 M2 token %0d full-block cycles %0d attn_checksum %08x attn_residual_checksum %08x ln2_checksum %08x post_gelu_checksum %08x c_proj_checksum %08x final_checksum %08x final_sample0 %08x final_sample1 %08x",
    ...
    $fatal(1, "Timeout waiting for task6 M2 first-token full-block done");
  end
endmodule
