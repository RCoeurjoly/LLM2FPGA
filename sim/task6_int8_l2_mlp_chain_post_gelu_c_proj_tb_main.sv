...
module task6_int8_l2_mlp_chain_post_gelu_c_proj_tb;
  ...
  initial begin : init_control
    ...
    for (out_index = 0; out_index < C_PROJ_OUT_DIM; out_index = out_index + 1) begin
      ...
      @(posedge clock);
      #1;
      if (output_read_data !== expected_c_proj_acc_values[out_index]) begin
        $display(
          "FAIL: mlp chain read addr %0d expected %0d got %0d",
          out_index,
          expected_c_proj_acc_values[out_index],
          output_read_data
        );
        $fatal(1);
      end
    ...
    $display(
      "PASS: task6 int8 L2 mlp chain postgelu c_proj reads %0d outputs %0d compute_cycles %0d total_cycles %0d",
      read_count,
      C_PROJ_OUT_DIM,
      compute_cycles,
      cycles
  ...
  always_ff @(posedge clock) begin
    if (!reset) begin
      ...
      if (cycles > TIMEOUT_CYCLES)
        $fatal(1, "Timeout waiting for task6 int8 L2 mlp chain postgelu c_proj completion");
    end
  end
endmodule
