...
module task6_int8_gemv64_lanes4_packed_tb;
  localparam int IN_DIM = 64;
  localparam int OUT_DIM = 64;
  localparam int LANES = 4;
  localparam int OUT_TILES = OUT_DIM / LANES;
  localparam int PACKED_WEIGHT_WORDS = OUT_TILES * IN_DIM;
  ...
  wire [9:0] packed_weight_addr;
  logic [LANES * 8 - 1:0] packed_weight_data;
  wire [5:0] out_addr;
  ...
  task6_int8_gemv64_lanes4_packed_kernel dut(
    .clock(clock),
    .reset(reset),
    .start(start),
    .busy(busy),
    ...
    .packed_weight_data(packed_weight_data),
    .out_addr(out_addr),
    .out_data(out_data),
    .out_valid(out_valid),
    .out_ready(out_ready)
  ...
  assign packed_weight_data = packed_weight_mem[packed_weight_addr];
  ...
  initial begin : init_memories
    ...
    for (tile_index = 0; tile_index < OUT_TILES; tile_index = tile_index + 1) begin
      for (in_index = 0; in_index < IN_DIM; in_index = in_index + 1) begin
        packed_weight_mem[tile_index * IN_DIM + in_index] = '0;
        for (lane_index = 0; lane_index < LANES; lane_index = lane_index + 1) begin
          packed_weight_mem[tile_index * IN_DIM + in_index][lane_index * 8 +: 8] =
            8'(weight_value(tile_index * LANES + lane_index, in_index));
        end
      end
    end
  end
  ...
  always_ff @(posedge clock) begin
    if (!reset) begin
      ...
      if (cycles > TIMEOUT_CYCLES)
        $fatal(1, "Timeout waiting for task6 int8 GEMV4 packed completion");
      ...
      if (done) begin
        ...
        $display(
          "PASS: task6 int8 GEMV4 packed stores %0d outputs %0d cycles %0d",
          observed_count,
          OUT_DIM,
          cycles
        );
