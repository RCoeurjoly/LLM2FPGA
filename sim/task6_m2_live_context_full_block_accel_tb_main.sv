...
module task6_m2_live_context_full_block_accel_tb;
  `include "tb_data.sv"
  `include "task6_m2_ln_attn_live_kv_all_heads_context_tb_data.sv"

  localparam int TIMEOUT_CYCLES = 120000;
  ...
  task6_m2_live_context_full_block_accel_top dut (
    .SYS_CLK(SYS_CLK),
    .SYS_RSTN(SYS_RSTN),
    .start_i(start_i),
  ...
  initial begin
    ...
    for (int dim = 0; dim < OUT_PROJ_DIM; dim = dim + 1) begin
      external_block_input_vector_i[dim * 8 +: 8] = out_proj_block_input_q[dim];
    end
    ...
    while (cycles < TIMEOUT_CYCLES) begin
      ...
      if (status_o[2]) begin
        $fatal(
          1,
          "FAIL: task6 M2 live-context full-block error status=%08x debug=%08x",
          status_o,
          debug_o
        );
      end

      if (status_o[7:4] == ST_DONE && status_o[3]) begin
        ...
        $display(
          "PASS: task6 M2 token %0d live-context full-block cycles %0d context_checksum %08x attn_checksum %08x attn_residual_checksum %08x ln2_checksum %08x post_gelu_checksum %08x c_proj_checksum %08x final_checksum %08x final_sample0 %08x final_sample1 %08x",
    ...
    $fatal(1, "Timeout waiting for task6 M2 live-context full-block done");
  end
endmodule
