...
module task6_m2_token_live_context_full_block_accel_tb;
  `include "tb_data.sv"
  ...
  task6_m2_token_live_context_full_block_accel_top dut (
    .SYS_CLK(SYS_CLK),
    .SYS_RSTN(SYS_RSTN),
    .start_i(start_i),
  ...
  initial begin
    ...
    for (int token = 0; token < TOKEN_BLOCK_SEQ; token = token + 1) begin
      token_ids_i[token * 16 +: 16] = token_block_expected_token_ids[token];
    end
    for (int dim = 0; dim < TOKEN_BLOCK_DIM; dim = dim + 1) begin
      expected_block_input_checksum =
        expected_block_input_checksum + ({24'd0, token_block_last_input_q[dim][7:0]} * (dim + 1));
    end
    ...
    while (cycles < TIMEOUT_CYCLES) begin
      ...
      if (status_o[2]) begin
        $fatal(
          1,
          "FAIL: task6 M2 token-live-context full-block error status=%08x debug=%08x",
          status_o,
          debug_o
        );
      end
      ...
      if (status_o[6:4] == ST_DONE && status_o[3]) begin
        ...
        $display(
          "PASS: task6 M2 token-live-context full-block cycles %0d block_input_checksum %08x context_checksum %08x attn_checksum %08x attn_residual_checksum %08x ln2_checksum %08x post_gelu_checksum %08x c_proj_checksum %08x final_checksum %08x final_sample0 %08x final_sample1 %08x",
          cycle_count_o,
    ...
    $fatal(1, "Timeout waiting for task6 M2 token-live-context full-block done");
  end
endmodule
