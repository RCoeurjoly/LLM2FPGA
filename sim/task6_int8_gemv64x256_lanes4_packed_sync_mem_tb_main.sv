...
module task6_int8_gemv64x256_lanes4_packed_sync_mem_tb;
  ...
  task6_int8_gemv64x256_lanes4_packed_sync_mem_kernel dut(
    .clock(clock),
    .reset(reset),
  ...
  initial begin : init_control
    ...
    for (
      ...
      for (in_index = 0; in_index < IN_DIM; in_index = in_index + 1) begin
        load_addr = 12'(output_group_index * IN_DIM + in_index);
        load_data = packed_weight_value(output_group_index, in_index);
        load_valid = 1'b1;
        @(negedge clock);
      end
    end
  ...
  always_ff @(posedge clock) begin
    if (!reset) begin
      ...
      if (cycles > TIMEOUT_CYCLES)
        $fatal(1, "Timeout waiting for task6 int8 GEMV4x256 syncmem completion");
      ...
      if (done) begin
        ...
        $display(
          "PASS: task6 int8 GEMV4x256 syncmem stores %0d outputs %0d cycles %0d",
          observed_count,
          OUT_DIM,
          cycles
        );
