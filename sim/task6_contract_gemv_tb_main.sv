...
`else
    $dumpfile("wave.fst");
`endif
    $dumpvars(0, task6_contract_gemv_tb);
  end
`endif

  // Scale the timeout with the external traffic size so larger redirected
  // kernels do not fail before they finish draining their load/store streams.
  localparam int TIMEOUT_CYCLES =
    1000 + ((ACTIVATION_WORDS + WEIGHT_WORDS + EXPECTED_STORE_COUNT) * 128);
  localparam real ABS_TOL = 1.0e-4;

  typedef struct packed {
    logic [OUTPUT_ADDR_WIDTH - 1:0] address;
    logic [31:0] data;
  } store_word_t;
    ...
    begin
      ...
      for (index = 0; index < EXPECTED_STORE_COUNT; index = index + 1) begin
        if (!seen_mem[index]) begin
          ...
          expected_val = $bitstoshortreal(expected_mem[index]);
          observed_val = $bitstoshortreal(observed_mem[index]);
          abs_error = observed_val - expected_val;
          if (abs_error < 0.0)
            abs_error = -abs_error;
          if (abs_error > ABS_TOL) begin
            mismatch_count = mismatch_count + 1;
            $display(
              "FAIL: addr %0d expected 0x%08x got 0x%08x abs_error=%e",
              index,
              expected_mem[index],
              observed_mem[index],
              abs_error
  ...
  always_ff @(posedge clock) begin
    if (reset) begin
      ...
      if (cycles > TIMEOUT_CYCLES)
        $fatal(1, "Timeout waiting for redirected GEMV completion");
    end
  end
endmodule
