...
module task6_int8_l2_c_fc_contract_local_io_tb;
  `include "tb_data.sv"
  ...
  initial begin : init_control
    ...
    $display(
      "PASS: task6 int8 L2 c_fc localio reads %0d outputs %0d compute_cycles %0d total_cycles %0d",
      read_count,
      OUT_DIM,
      compute_cycles,
      cycles
  ...
  always_ff @(posedge clock) begin
    if (!reset) begin
      ...
      if (cycles > TIMEOUT_CYCLES)
        $fatal(1, "Timeout waiting for task6 int8 L2 c_fc localio completion");
    end
  end
endmodule
