...
module task6_m2_embedding_live_context_attention_slice_pcie_accel_tb;
  ...
  logic [511:0] expected_context_vector;
  logic [31:0] expected_context_checksum;
  logic [31:0] expected_context_sample0;
  logic [31:0] expected_context_sample1;
    ...
    begin
      ...
      while (cycles < TIMEOUT_CYCLES) begin
        ...
        if (pcie_status_o[2]) begin
          $fatal(
            1,
            "FAIL: task6 M2.4 context PCIe wrapper error pass=%0d status=%08x debug=%08x debug1=%08x debug2=%08x",
            pass_index,
            pcie_status_o,
            pcie_debug_o,
            pcie_debug1_o,
        ...
        if (status_state(pcie_status_o) == M2_DONE && pcie_status_o[3]) begin
          ...
          if (pcie_provenance_o !== (32'h4d32_4105)) begin
            $fatal(
              1,
          ...
          if (pcie_output_checksum_o !== expected_context_checksum) begin
            $fatal(
              1,
              "FAIL: context checksum expected %08x got %08x pass=%0d",
              expected_context_checksum,
              pcie_output_checksum_o,
              pass_index
            );
          end
          if (pcie_output_sample0_o !== expected_context_sample0) begin
            $fatal(
              1,
              "FAIL: context sample0 expected %08x got %08x",
              expected_context_sample0,
              pcie_output_sample0_o
            );
          end
          if (pcie_output_sample1_o !== expected_context_sample1) begin
            $fatal(
              1,
              "FAIL: context sample1 expected %08x got %08x",
              expected_context_sample1,
          ...
          if (pcie_output_vector_o !== expected_context_vector) begin
            $fatal(1, "FAIL: context vector mismatch pass=%0d", pass_index);
          end
      ...
      $fatal(1, "Timeout waiting for task6 M2.4 context PCIe wrapper done");
    end
  endtask
  ...
  initial begin
    ...
    pcie_start_pulse_i = 1'b0;
    pcie_clear_pulse_i = 1'b0;
    expected_context_vector = 512'd0;
    expected_context_checksum = 32'd0;
    expected_context_sample0 = 32'd0;
    expected_context_sample1 = 32'd0;
    ...
    for (int dim = 0; dim < 64; dim = dim + 1) begin
      expected_context_vector[dim * 8 +: 8] = context_expected_q[dim];
      expected_context_checksum =
        expected_context_checksum + ({24'd0, context_expected_q[dim][7:0]} * (dim + 1));
      if (dim < 4) begin
        expected_context_sample0 =
          expected_context_sample0 | ({24'd0, context_expected_q[dim][7:0]} << (8 * dim[1:0]));
      end else if (dim < 8) begin
        expected_context_sample1 =
          expected_context_sample1 | ({24'd0, context_expected_q[dim][7:0]} << (8 * dim[1:0]));
      end
    ...
    $display(
      "PASS: task6 M2.4 embedding live-context attention-slice PCIe wrapper cycles %0d context_checksum %08x context_sample0 %08x context_sample1 %08x debug %08x debug1 %08x debug3 %08x provenance %08x",
      pcie_cycle_count_o,
      pcie_output_checksum_o,
      pcie_output_sample0_o,
      pcie_output_sample1_o,
