...
module task6_int8_gemv64_lanes4_kernel #(
  ...
  output logic [LANES * WEIGHT_ADDR_WIDTH - 1:0] weight_addr,
  ...
  localparam logic [WEIGHT_ADDR_WIDTH - 1:0] WEIGHT_TILE_STRIDE =
    WEIGHT_ADDR_WIDTH'(LANES * IN_DIM);
  ...
  logic [WEIGHT_ADDR_WIDTH - 1:0] weight_base_q;
  logic [LANE_ADDR_WIDTH - 1:0] emit_lane_q;
  logic [LANE_ADDR_WIDTH - 1:0] emit_lane_next_w;
  logic [WEIGHT_ADDR_WIDTH - 1:0] in_index_ext_w;
  ...
  assign in_index_ext_w = WEIGHT_ADDR_WIDTH'(in_index_q);

  genvar lane;
  generate
    for (lane = 0; lane < LANES; lane = lane + 1) begin : gen_lane
      assign weight_addr[lane * WEIGHT_ADDR_WIDTH +: WEIGHT_ADDR_WIDTH] =
        weight_base_q + WEIGHT_ADDR_WIDTH'(lane * IN_DIM) + in_index_ext_w;
      assign product_w[lane] =
        activation_data * $signed(weight_data[lane * 8 +: 8]);
          ...
          if (out_valid && out_ready) begin
            if (emit_lane_q == LAST_LANE_INDEX) begin
              ...
              if (out_base_q == LAST_OUT_BASE) begin
                ...
                weight_base_q <= weight_base_q + WEIGHT_TILE_STRIDE;
                state_q <= STATE_RUN;
              end
            end else begin
