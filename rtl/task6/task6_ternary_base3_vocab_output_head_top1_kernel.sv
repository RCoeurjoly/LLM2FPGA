...
module task6_ternary_base3_vocab_output_head_top1_kernel #(
  parameter int IN_DIM = 64,
  parameter int VOCAB_SIZE = 10000,
  parameter int VALID_VOCAB_SIZE = VOCAB_SIZE,
  parameter int TILE_OUT_DIM = 80,
  parameter int WEIGHTS_PER_WORD = 20,
  ...
  function automatic logic [1:0] base3_code(
    input logic [31:0] word,
    input int lane
  );
    unique case (lane)
      0:  base3_code = 2'((word / 1) % 3);
      1:  base3_code = 2'((word / 3) % 3);
      2:  base3_code = 2'((word / 9) % 3);
      3:  base3_code = 2'((word / 27) % 3);
      4:  base3_code = 2'((word / 81) % 3);
      5:  base3_code = 2'((word / 243) % 3);
      6:  base3_code = 2'((word / 729) % 3);
      7:  base3_code = 2'((word / 2187) % 3);
      8:  base3_code = 2'((word / 6561) % 3);
      9:  base3_code = 2'((word / 19683) % 3);
      10: base3_code = 2'((word / 59049) % 3);
      11: base3_code = 2'((word / 177147) % 3);
      12: base3_code = 2'((word / 531441) % 3);
      13: base3_code = 2'((word / 1594323) % 3);
      14: base3_code = 2'((word / 4782969) % 3);
      15: base3_code = 2'((word / 14348907) % 3);
      16: base3_code = 2'((word / 43046721) % 3);
      17: base3_code = 2'((word / 129140163) % 3);
      18: base3_code = 2'((word / 387420489) % 3);
      default: base3_code = 2'((word / 1162261467) % 3);
    endcase
  endfunction

  function automatic signed [1:0] decode_base3_ternary(
    input logic [31:0] word,
    input int lane
  );
    logic [1:0] code;
    begin
      code = base3_code(word, lane);
      unique case (code)
        2'd1: decode_base3_ternary = 2'sd1;
        2'd2: decode_base3_ternary = -2'sd1;
        default: decode_base3_ternary = 2'sd0;
      endcase
    end
  endfunction
    ...
    if (reset) begin
      ...
      if (apply_valid_q) begin
        for (int lane = 0; lane < WEIGHTS_PER_WORD; lane = lane + 1) begin
          unique case (decode_base3_ternary(packed_weight_data_q, lane))
            2'sd1: begin
