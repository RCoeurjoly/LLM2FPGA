...
def render_litedram_yaml(config: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "# LiteDRAM config for the YPCB DDR3 CH0 72-bit physical bus.",
        "# The first 72-bit probe uses the native controller width; the later",
        "# rowstream bridge must pack or ignore the ECC byte lane explicitly.",
...
def main() -> None:
    ...
    scalars = ucf["scalars"]
    ddram: dict[str, list[str] | str] = {
        "dq": pins_for(indexed["ddr3_dq"], 72),
        "dqs_p": pins_for(indexed["ddr3_dqs_p"], 9),
        "dqs_n": pins_for(indexed["ddr3_dqs_n"], 9),
    ...
    payload = {
        ...
        "decision": {
            ...
            "next_gate": (
                "Instantiate a minimal custom LiteX/YPCB no-ODELAY DDR3 target with "
