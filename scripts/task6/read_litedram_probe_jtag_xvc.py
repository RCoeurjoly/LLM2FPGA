...
FIELDS = [
    ...
    ("native_cmdaddr_first_flags", 11168, 8),
    ("native_cmdaddr_first_command_index", 11176, 8),
    ("native_cmdaddr_first_scheduled", 11184, 25),
    ("native_cmdaddr_first_presented", 11209, 25),
    ("native_cmdaddr_first_accepted", 11234, 25),
    ("native_cmdaddr_first_mode_flags", 11259, 5),
]
...
def decode_native_address_classifier(fields: dict[str, int]) -> dict[str, object] | None:
    ...
    single_cmdaddr_trace = fields.get("version", 0) >= 116
    compact_cmdaddr_trace = fields.get("version", 0) == 115
    cmdaddr_prefix = (
        "native_cmdaddr_compact" if compact_cmdaddr_trace else "native_cmdaddr"
    )
    if single_cmdaddr_trace:
        cmdaddr_first_flags = fields.get("native_cmdaddr_first_flags", 0)
        cmdaddr_presented_valid_count = cmdaddr_first_flags & 0x1
        cmdaddr_accepted_valid_count = (cmdaddr_first_flags >> 1) & 0x1
    else:
    ...
    cmdaddr_trace = []
    if single_cmdaddr_trace and cmdaddr_accepted_valid_count:
        scheduled = fields.get("native_cmdaddr_first_scheduled", 0)
        presented = fields.get("native_cmdaddr_first_presented", 0)
        accepted = fields.get("native_cmdaddr_first_accepted", 0)
        command_index = fields.get("native_cmdaddr_first_command_index", 0)
        cmdaddr_trace.append(
            {
                ...
                "accepted_matches_requested": (
                    accepted == native_address_classifier_addr(command_index)
                ),
            }
        )
    elif fields.get("version", 0) >= 114:
    ...
    return {
        ...
        "cmdaddr_flags": (
            fields.get("native_cmdaddr_first_flags", 0)
            if single_cmdaddr_trace
            else fields.get(f"{cmdaddr_prefix}_flags", 0)
        ),
        "cmdaddr_compact_trace": compact_cmdaddr_trace,
        "cmdaddr_single_trace": single_cmdaddr_trace,
    ...
    if fields.get("version", 0) < 49 or fields.get("version", 0) >= 54:
        print(
            ...
            "data_pass={data_pass}".format(
                ...
                mask=fields.get("dfii_word_mismatch_mask", 0)
                & (0xFFFFF if fields.get("version", 0) >= 83 else 0xFFFF),
    ...
    if (
        ...
        if lane7_locator_probe_only:
            ...
            if edge_comp_addrwalk_mode:
                ...
                result = "PASS" if full_mask == 0 else "FAIL"
                print(
                    "dfii compensated-edge active probe: all-phase "
                    "lane7/lane8 final-word compensation promoted to the "
                    "single active DFII write/read path"
                )
                print(
                    f"  active: dram_mismatch=0x{full_mask:05x} {result}"
                )
            elif fields.get("version", 0) >= 77:
                masks = decoded["dfii_phasecmd_mismatch_masks"]
                print(
                ...
                if version == 78:
                    ...
                    print(
                        "dfii compensated-edge probe: DRAM variant sweep with "
                        "A7 PHY DFI wrdata word4 taps and packed full "
                        "20-word masks"
                    )
                elif version >= 81:
                    print(
                        "dfii compensated-edge probe: DRAM variant sweep with "
                        "A7 PHY DFI wrdata word4 taps and full 20-word masks"
                    )
                elif version >= 80:
                    print(
                        "dfii compensated-edge probe: DRAM variant sweep with "
                        "A7 PHY DFI wrdata word4 debug taps"
                    )
                ...
                dfi_mask_high_masks = fields.get(
                    "dfii_half_low_match_high_masks", 0
                )
                edge_comp_high_mismatch_masks = fields.get(
                    "dfii_half_high_match_high_masks", 0
                )
                for index, label in enumerate(labels):
                    mask = masks[index]["mismatch_mask"]
                    high_mismatch = (
                        (edge_comp_high_mismatch_masks >> (index * 4)) & 0xF
                        if version >= 81
                        else 0
                    )
                    full_mask = mask | (high_mismatch << 16)
                    dram_result = "PASS" if full_mask == 0 else "FAIL"
                    if version == 79:
                        ...
                        flags = fields.get(f"dfii_assoc_nonzero_mask_{index}", 0)
                        dfi_seen = flags & 0xF
                        dfi_last_en = (flags >> 4) & 0xF
                        dfi_events = (flags >> 8) & 0xF
                        high_mismatch = (
                            (flags >> 12) & 0xF
                            if version >= 82
                            else high_mismatch
                        ...
                        dram_result = "PASS" if full_mask == 0 else "FAIL"
                        dfi_mask = (
                            ((csr_echo_high_masks >> (index * 4)) & 0xF)
                            | (((dfi_mask_high_masks >> (index * 4)) & 0xF) << 4)
                        )
                        dfi_word4 = [
                            fields.get(
                                f"dfii_assoc_match_mask_{index * 4 + phase}",
                                0,
                            )
                            & 0xFFFF
                            for phase in range(4)
                        ...
                        dfi_result = (
                            "PASS" if dfi_word4 == dfi_expected else "FAIL"
                        )
                        dfi_words = " ".join(
                            f"p{phase}:0x{word:04x}/exp=0x{dfi_expected[phase]:04x}"
                            for phase, word in enumerate(dfi_word4)
                        )
                        if dfii_complete:
                            print(
                                f"  variant{index}: {label}: "
                                f"dram_mismatch=0x{full_mask:05x} "
                                f"{dram_result}; "
                                f"dfi_word4={dfi_result} seen=0x{dfi_seen:x} "
                                f"last_en=0x{dfi_last_en:x} events={dfi_events} "
                                f"mask=0x{dfi_mask:02x}; {dfi_words}"
                            )
                        else:
                            print(
                                f"  variant{index}: {label}: "
                                f"dram_mismatch=0x{full_mask:05x}; "
                                f"dfi_seen=0x{dfi_seen:x} "
                                f"dfi_last_en=0x{dfi_last_en:x}; invalid"
                            )
                    elif version >= 78:
