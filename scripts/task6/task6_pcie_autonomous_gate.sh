...
usage() {
  cat >&2 <<'EOF'
usage: scripts/task6/task6_pcie_autonomous_gate.sh <bar|m2-full-block|rowstream-loopback|command|command-header|command-echo|command-doorbell> [BDF] [--run-dir DIR] [--repeat N]
EOF
  exit 2
}
...
[[ -n "$MODE" ]] || usage
case "$MODE" in
  bar|m2-full-block|rowstream-loopback|command|command-header|command-echo|command-doorbell) ;;
  *) usage ;;
esac
shift || true
if [[ "${1:-}" != --* && -n "${1:-}" ]]; then
