...
for _ in $(seq 1 20); do
  if lspci -Dnn -s "$BDF" >/tmp/task6-pcie-rescan-lspci.$$ 2>&1; then
    if [[ -s /tmp/task6-pcie-rescan-lspci.$$ ]]; then
      ...
      if grep -qi 'Unknown header type 7f' <<<"$detail"; then
        ...
        endpoint="$(cat /tmp/task6-pcie-rescan-lspci.$$)"
        break
      fi
    fi
  fi
...
rm -f /tmp/task6-pcie-rescan-lspci.$$
...
lspci -vv -s "$BDF"
exec "$ROOT/scripts/task6/task6_pcie_bar_smoke.sh" "$BDF"
