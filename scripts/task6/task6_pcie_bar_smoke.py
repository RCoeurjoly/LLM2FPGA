#!/usr/bin/env python3
"""BAR0 read/write smoke for the pcie_7x Vivado oracle endpoint.
...
def bar0_worker(resource_path: str, size: int, pattern: bytes, conn: mp.connection.Connection) -> None:
    ...
    try:
        try:
            with mmap.mmap(fd, size, mmap.MAP_SHARED, mmap.PROT_READ | mmap.PROT_WRITE) as mm:
                ...
                mm.flush()
                mm.seek(0)
                readback = mm.read(max(64, len(pattern)))
                conn.send(("ok", "mmap", initial, readback))
                return
        except PermissionError as exc:
            conn.send(("mmap-denied", str(exc)))

        initial = os.pread(fd, 4, 0)
        written = os.pwrite(fd, pattern, 0)
        if written != len(pattern):
            conn.send(("error", f"short BAR0 write: wrote {written} of {len(pattern)} bytes"))
            return
        readback = os.pread(fd, max(64, len(pattern)), 0)
        conn.send(("ok", "pread/pwrite", initial, readback))
...
def run_bar0_smoke(resource0: Path, size: int, pattern: bytes, timeout_s: float) -> tuple[str, bytes, bytes, list[str]]:
    parent_conn, child_conn = mp.Pipe(duplex=False)
    proc = mp.Process(target=bar0_worker, args=(str(resource0), size, pattern, child_conn))
    proc.start()
    child_conn.close()
    ...
    try:
        ...
        if msg[0] == "ok":
            _, method, initial, readback = msg
            return method, initial, readback, messages
...
def main() -> int:
    ...
    parser.add_argument(
        "--pattern",
        default="abcdefgh",
        help="ASCII bytes to write at BAR0 offset 0",
    )
    parser.add_argument(
        "--no-direct-config",
        action="store_true",
    ...
    method, initial, readback, messages = run_bar0_smoke(
        resource0, args.size, pattern, args.bar_timeout
    )
    for message in messages:
        print(message)
    print(f"BAR0 first word via {method}:")
    print(hexdump(initial))
    print("BAR0 write/readback smoke:")
    print(hexdump(readback[:64]))
    if readback[: len(pattern)] != pattern:
        raise SystemExit(
            f"BAR0 readback mismatch: expected {pattern!r}, got {readback[:len(pattern)]!r}"
        )
