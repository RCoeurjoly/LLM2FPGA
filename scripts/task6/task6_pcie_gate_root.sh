...
usage() {
  cat >&2 <<'EOF'
usage: task6-pcie-gate <prepare|bar|rowstream-loopback|rowstream-loader|rowstream-packet|rowstream-run|command|command-header|command-echo|command-doorbell> [0000:42:00.0] [mode args...]
...
BDF="${2:-$ALLOWED_BDF}"
case "$MODE" in
  prepare|bar|rowstream-loopback|rowstream-loader|rowstream-packet|rowstream-run|command|command-header|command-echo|command-doorbell) ;;
  *) usage ;;
esac
