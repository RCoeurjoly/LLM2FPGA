...
use_lsq="${HANDSHAKE_USE_LSQ:-0}"
extra=()
if [[ "$insert_buffers" == "1" || "$insert_buffers" == "true" || "$insert_buffers" == "yes" ]]; then
  extra+=( -handshake-insert-buffers )
fi

lower_cf_to_handshake=( --lower-cf-to-handshake )
if [[ "$use_lsq" == "1" || "$use_lsq" == "true" || "$use_lsq" == "yes" ]]; then
  lower_cf_to_handshake=( --lower-cf-to-handshake=lsq )
fi
