...
sv_split_only="${SV_SPLIT_ONLY:-0}"

if [[ "$sv_split_only" == "1" ]] && [[ -z "$sv_split_dir" ]]; then
  echo "[hw_clean_to_sv] ERROR: SV_SPLIT_ONLY=1 requires SV_SPLIT_DIR." >&2
  exit 1
fi
...
if [[ "$sv_split_only" != "1" ]]; then
  "$CIRCT_OPT" "$input" \
    "${common_passes[@]}" \
    -export-verilog >"$output"
