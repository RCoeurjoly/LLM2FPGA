...
let
  ...
  mkCfDerivation =
    { name, linalg, linalgLowering ? "affine", cfRequireNoFloat ? false }:
  ...
  mkHandshakeDerivation = { name, cf, handshakeInsertBuffers ? true
    , handshakeUseLsq ? false, circtPkg ? circt }:
    pkgs.runCommand "${name}-handshake.mlir" {
      ...
      if [ "${if handshakeUseLsq then "1" else "0"}" = "1" ]; then
        export HANDSHAKE_USE_LSQ=1
      else
        export HANDSHAKE_USE_LSQ=0
      fi
  ...
  mkPipeline = { name, torchMlirInput, linalgLowering ? "affine"
    ...
    , handshakeUseLsq ? false, allowHwExterns ? false, useSplitSvForIl ? false
    , circtPkg ? circt }: rec {
      torch = mkTorchDerivation { inherit name torchMlirInput; };
      linalg = mkLinalgDerivation { inherit name torch; };
      cf =
        mkCfDerivation { inherit name linalg linalgLowering cfRequireNoFloat; };
      cfStats = mkCfStatsDerivation { inherit name cf; };
      handshake = mkHandshakeDerivation {
        inherit name cf handshakeInsertBuffers handshakeUseLsq circtPkg;
      };
      ...
      svSplit = if useSplitSvForIl then
        ...
        null;
      sv = if useSplitSvForIl then
        svSplit
      else
        mkSvDerivation { inherit name hwClean allowHwExterns circtPkg; };
      il = mkIlDerivation {
        inherit name;
    ...
    pkgs.writeText "${modelKey}-pipeline-metadata.json" (builtins.toJSON {
      model = {
        ...
        inherit (model)
          ...
          handshakeInsertBuffers handshakeUseLsq allowHwExterns useSplitSvForIl;
      };
  ...
  registerModel = { name, key ? name, description ? ""
    ...
    , handshakeInsertBuffers ? true, handshakeUseLsq ? false
    , allowHwExterns ? false, useSplitSvForIl ? false, circtPkg ? circt }:
    let
      ...
      pipeline = mkPipeline {
        inherit name linalgLowering cfRequireNoFloat handshakeInsertBuffers
          handshakeUseLsq allowHwExterns useSplitSvForIl circtPkg;
