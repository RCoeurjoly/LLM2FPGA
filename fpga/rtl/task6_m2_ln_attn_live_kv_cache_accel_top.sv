...
module task6_m2_ln_attn_live_kv_cache_accel_top (
  input logic SYS_CLK,
  ...
  assign k_proj_output_product_w =
    ...
    $signed(k_proj_output_mul_q20_by_token[token_index_q][dim_index_q]);
  assign v_proj_output_product_w =
    $signed(v_proj_next_acc_w) *
    $signed(v_proj_output_mul_q20_by_token[token_index_q][dim_index_q]);
  assign q_proj_output_product_w =
    $signed(q_proj_next_acc_w) *
    $signed(q_proj_output_mul_q20_final[dim_index_q]);
  ...
  assign score_delta_scaled_w =
    ...
    $signed(SOFTMAX_SCORE_SCALE_Q20);
      ...
      unique case (state_q)
        ...
        S_RUN_LN: begin
          ...
          if (ln_output_w != ln_expected_q_by_token[token_index_q][ln_index_q]) begin
            state_q <= S_ERROR;
            debug_q <= {8'h01, token_index_u32_w[3:0], ln_index_q, 6'd0, ln_output_w};
          end else begin
        ...
        S_RUN_PROJ: begin
          ...
          if (proj_index_q == LN_INDEX_WIDTH'(LN_DIM - 1)) begin
            if (k_proj_output_w != k_proj_expected_q_by_token[token_index_q][dim_index_q]) begin
              state_q <= S_ERROR;
              debug_q <= {8'h02, token_index_u32_w[3:0], dim_index_u32_w[3:0], 8'd0, k_proj_output_w};
            end else if (v_proj_output_w != v_proj_expected_q_by_token[token_index_q][dim_index_q]) begin
              state_q <= S_ERROR;
              debug_q <= {8'h03, token_index_u32_w[3:0], dim_index_u32_w[3:0], 8'd0, v_proj_output_w};
            end else if (
              token_index_q == TOKEN_WIDTH'(CACHE_SEQ - 1) &&
              q_proj_output_w != q_proj_expected_q_final[dim_index_q]
            ) begin
              state_q <= S_ERROR;
              debug_q <= {8'h04, token_index_u32_w[3:0], dim_index_u32_w[3:0], 8'd0, q_proj_output_w};
            end else begin
        ...
        S_RUN_SCORE: begin
          cycle_count_q <= cycle_count_q + 32'd1;
          if (score_acc_w != attn_expected_score_acc[src_index_q]) begin
            state_q <= S_ERROR;
            debug_q <= {8'h05, 4'd0, src_index_u32_w[3:0], score_acc_w[15:0]};
        ...
        S_RUN_PROB_WEIGHT: begin
          cycle_count_q <= cycle_count_q + 32'd1;
          if (softmax_weight_w != {16'd0, attn_expected_weight_q15[src_index_q]}) begin
            state_q <= S_ERROR;
            debug_q <= {8'h07, 4'd0, src_index_u32_w[3:0], softmax_weight_w[15:0]};
          end else if (src_index_q == TOKEN_WIDTH'(CACHE_SEQ - 1)) begin
        ...
        S_RUN_PROB_NORM: begin
          cycle_count_q <= cycle_count_q + 32'd1;
          if (prob_current_w != attn_prob_q15[src_index_q]) begin
            state_q <= S_ERROR;
            debug_q <= {8'h08, 4'd0, src_index_u32_w[3:0], prob_current_w};
          end else if (src_index_q == TOKEN_WIDTH'(CACHE_SEQ - 1)) begin
        ...
        S_RUN_VALUE: begin
          cycle_count_q <= cycle_count_q + 32'd1;
          if (
            value_acc_w != attn_expected_value_acc[dim_index_q] ||
            value_q_w != attn_expected_value_q[dim_index_q]
          ) begin
            state_q <= S_ERROR;
            debug_q <= {8'h06, 4'd0, dim_index_u32_w[3:0], value_acc_w[15:0]};
          end else begin
            output_checksum_o <=
              output_checksum_o +
              ({24'd0, value_q_w[7:0]} * (dim_index_u32_w + 32'd1));
            output_vector_o[dim_index_q * 8 +: 8] <= value_q_w;
            if (dim_index_u32_w < 32'd4) begin
              output_sample0_o <=
                output_sample0_o | ({24'd0, value_q_w[7:0]} << (8 * dim_index_q[1:0]));
            end else if (dim_index_u32_w < 32'd8) begin
              output_sample1_o <=
                output_sample1_o | ({24'd0, value_q_w[7:0]} << (8 * dim_index_q[1:0]));
            end
