module task6_ypcb_litedram_init_bandwidth_probe_top #(
  ...
  wire dfii_byte_phase_assoc_matrix_mode;
  ...
  wire native_cmdaddr_trace_mode;
  ...
  logic [15:0] dfii_byte_phase_nonzero_mask_next;
  logic [15:0] dfii_byte_phase_match_mask_next;
  logic [3:0] dfii_byte_phase_nonzero_high_mask_next;
  logic [3:0] dfii_byte_phase_match_high_mask_next;
  ...
  function automatic logic [7:0] dfii_byte_phase_assoc_tag_byte(
    input logic [3:0] slot
  );
    begin
      dfii_byte_phase_assoc_tag_byte = 8'ha0 + {4'd0, slot};
    end
  endfunction

  function automatic logic [31:0] dfii_byte_phase_assoc_word(
    input logic [3:0] slot,
    input logic [1:0] word
  );
    logic [7:0] tag;
    begin
      ...
      dfii_byte_phase_assoc_word = 32'd0;
      if (word == slot[3:2]) begin
        unique case (slot[1:0])
          2'd0: dfii_byte_phase_assoc_word = {24'd0, tag};
          2'd1: dfii_byte_phase_assoc_word = {16'd0, tag, 8'd0};
          2'd2: dfii_byte_phase_assoc_word = {8'd0, tag, 16'd0};
          default: dfii_byte_phase_assoc_word = {tag, 24'd0};
        endcase
      end
    end
  endfunction

  function automatic logic [31:0] dfii_byte_phase_assoc_word5(
    input logic [3:0] slot,
    input logic [2:0] word
  );
    logic [7:0] tag;
    ...
    begin
      tag = dfii_byte_phase_assoc_tag_byte(slot);
      placement_slot = (slot == 4'd7) ? 4'd8 : slot;
      ...
      if (word == {1'b0, placement_slot[3:2]}) begin
        unique case (placement_slot[1:0])
          ...
          default: dfii_byte_phase_assoc_word5 = {tag, 24'd0};
        endcase
      end
    end
  endfunction
  ...
  function automatic logic [575:0] native_dfii_addrwalk_expected_rdata(
    input logic [24:0] addr
  );
    logic [3:0] addr_index;
    begin
      addr_index = addr[3:0];
      native_dfii_addrwalk_expected_rdata = 576'd0;
      for (int word_idx = 0; word_idx < 18; word_idx++) begin
        native_dfii_addrwalk_expected_rdata[word_idx * 32 +: 32] =
          dfii_edge_comp_addrwalk_read_index_word(
            addr_index,
            word_idx[4:0]
          );
  ...
  function automatic logic [4:0] dfii_phase_word20_index(
    input logic [1:0] phase,
    input logic [2:0] word
  );
    begin
      unique case (phase)
        2'd0: dfii_phase_word20_index = {2'd0, word};
        2'd1: dfii_phase_word20_index = 5'd5 + {2'd0, word};
        2'd2: dfii_phase_word20_index = 5'd10 + {2'd0, word};
        default: dfii_phase_word20_index = 5'd15 + {2'd0, word};
      endcase
    end
  endfunction

  function automatic logic [7:0] dfii_actual_byte20(
    input logic [1:0] phase,
    input logic [4:0] byte_index
  );
    logic [4:0] word_index;
    begin
      word_index = dfii_phase_word20_index(phase, byte_index[4:2]);
      dfii_actual_byte20 =
        byte_from_word(dfii_rddata_q[word_index], byte_index[1:0]);
    end
  endfunction
  ...
  assign response_mismatch =
    read_state && !byte_diag_read_state &&
    !(native_readscan_mode && !native_expected_read_mode) &&
    rdata_valid &&
  ...
  assign dfii_byte_phase_assoc_matrix_mode =
    DFII_BYTE_PHASE_ASSOC_MATRIX_ONLY && dfii_phasecmd_sweep_q;
  ...
  assign dfii_matrix_source_phase =
    (dfii_source_order_matrix_mode || dfii_byte_phase_assoc_matrix_mode ||
     dfii_half_order_matrix_mode || dfii_displacement_probe_mode ||
     dfii_csr_echo_probe_mode || dfii_edge_map_probe_mode ||
     dfii_edge_comp_probe_mode ||
     dfii_edge_comp_active_mode || dfii_edge_comp_bist_mode ||
     dfii_edge_comp_addrwalk_mode || dfii_edge_lane7_locator_probe_mode ||
  ...
  always_comb begin
    ...
    dfii_byte_phase_nonzero_mask_next = 16'd0;
    dfii_byte_phase_match_mask_next = 16'd0;
    dfii_byte_phase_nonzero_high_mask_next = 4'd0;
    dfii_byte_phase_match_high_mask_next = 4'd0;
    ...
    for (int byte_read_idx = 0;
         byte_read_idx < 16;
         byte_read_idx++) begin
      if (dfii_actual_byte20(
            dfii_read_command_phase,
            byte_read_idx[4:0]
          ) != 8'd0)
        dfii_byte_phase_nonzero_mask_next[byte_read_idx] = 1'b1;
      if (dfii_actual_byte20(
            dfii_read_command_phase,
            byte_read_idx[4:0]
          ) == dfii_byte_phase_assoc_tag_byte(dfii_phasecmd_index_q))
        dfii_byte_phase_match_mask_next[byte_read_idx] = 1'b1;
    end
    for (int byte_read_idx = 16;
         byte_read_idx < 20;
         byte_read_idx++) begin
      if (dfii_actual_byte20(
            dfii_read_command_phase,
        ...
        dfii_byte_phase_nonzero_high_mask_next[byte_read_idx - 16] = 1'b1;
      if (dfii_actual_byte20(
            dfii_read_command_phase,
            byte_read_idx[4:0]
          ) == dfii_byte_phase_assoc_tag_byte(dfii_phasecmd_index_q))
        dfii_byte_phase_match_high_mask_next[byte_read_idx - 16] = 1'b1;
    end
...
`endif
  ...
  always_ff @(posedge user_clk) begin
    ...
    end else if (|debug_dfi_wrdata_en) begin
  ...
  always_ff @(posedge user_clk) begin
    if (user_rst || core_rst) begin
      ...
      native_packing_valid_count_q <= 8'd0;
      for (int packing_idx = 0;
          ...
          if (timeout_seen || dfii_wb_timeout_seen_q) begin
            ...
            if (dfii_phasecmd_sweep_q) begin
              dfii_phasecmd_mismatch_q[dfii_result_slot] <=
                ...
                dfii_byte_phase_assoc_matrix_mode ?
                ~dfii_byte_phase_match_mask_next :
                dfii_edge_comp_probe_mode ?
              ...
              if (dfii_byte_phase_assoc_matrix_mode) begin
                dfii_assoc_nonzero_mask_q[dfii_result_slot] <=
                  dfii_byte_phase_nonzero_mask_next;
                dfii_assoc_match_mask_q[dfii_result_slot] <=
                  dfii_byte_phase_match_mask_next;
                dfii_half_nonzero_high_q[dfii_result_slot] <=
                  dfii_byte_phase_nonzero_high_mask_next;
                dfii_half_low_match_high_q[dfii_result_slot] <=
                  dfii_byte_phase_match_high_mask_next;
              end
        ...
        if (rdata_valid) begin
          ...
          if (native_readscan_mode && response_any_nonzero) begin
            ...
            if (native_nonzero_count_q == 32'd0) begin
              native_first_nonzero_addr_q <= compare_addr_q;
              native_first_nonzero_data_q <= first_nonzero_chunk_data(rdata);
              native_first_nonzero_chunk_q <= response_nonzero_chunk;
            end
          end
          if (native_packing_classifier_mode &&
              response_count_q < NATIVE_PACKING_SAMPLE_COUNT) begin
  ...
  always_comb begin
    ...
    if (DFII_EDGE_COMP_ADDRWALK_THEN_NATIVE) begin
    ...
    end else if (DFII_EDGE_COMP_ADDRWALK_THEN_NATIVE_PACKING_CLASSIFIER) begin
      jtag_debug_payload[1728 +: 8] = native_packing_valid_count_q;
      ...
      for (int packing_sample_idx = 0;
           packing_sample_idx < NATIVE_PACKING_SAMPLE_COUNT;
           packing_sample_idx++) begin
        for (int packing_chunk_idx = 0;
             packing_chunk_idx < 9;
             packing_chunk_idx++) begin
          jtag_debug_payload[
            1792 +
            (packing_sample_idx * 9 + packing_chunk_idx) * 64 +: 64
          ] =
            native_packing_rdata_q[packing_sample_idx][
              packing_chunk_idx * 64 +: 64
            ];
        end
      end
