...
module task6_m2_embedding_block_input_accel_top (
  ...
  assign ln_input_q12_full_w = round_shift_signed_64(added_q20_ext_w, EMBED_BLOCK_TO_LN_Q12_SHIFT);
  assign ln_input_q12_w = ln_input_q12_full_w[15:0];
  assign block_input_full_w = round_shift_signed_64(
    added_q20_ext_w * {{32{EMBED_BLOCK_REQUANT_MUL_Q20[31]}}, EMBED_BLOCK_REQUANT_MUL_Q20},
      ...
      unique case (state_q)
        ...
        S_EMIT_ADD: begin
          ...
          if (ln_input_q12_w != embed_block_expected_ln_input_q12_by_token[emit_token_index_q][dim_index_q]) begin
            state_q <= S_ERROR;
            debug_o <= {8'h02, emit_token_index_q, dim_index_q, 4'd0};
          end else if (emit_token_index_q == TOKEN_INDEX_WIDTH'(EMBED_BLOCK_SEQ - 1)) begin
            block_input_vector_o[dim_index_q * 8 +: 8] <= block_input_q_w;
            block_input_checksum_q <=
              block_input_checksum_q +
              ({24'd0, block_input_q_w[7:0]} * (dim_index_u32_w + 32'd1));
            if (block_input_q_w != embed_block_expected_last_input_q[dim_index_q]) begin
              state_q <= S_ERROR;
              debug_o <= {8'h03, 4'd0, dim_index_q, block_input_q_w};
