...
module task6_ypcb_uberddr3_user_port_probe_top #(
  ...
  localparam logic [WB_ADDR_BITS - 1:0] PROBE_BASE_ADDR =
    {{(WB_ADDR_BITS - 7) {1'b0}}, 7'd64};
  ...
  logic [7:0] read_probe_stream_index_q;
  logic [7:0] read_probe_next_stream_index_q;
  logic [7:0] read_probe_observed_byte_q;
  logic [9:0] read_probe_write_drain_q;
  logic [31:0] read_probe_wait_cycles_q;
  logic [WB_ADDR_BITS - 1:0] read_probe_addr;
  logic [WB_DATA_BITS - 1:0] read_probe_expected_data;
  logic [7:0] read_probe_expected_byte;
  logic [7:0] read_probe_low_observed_byte;
  logic read_probe_match_q;
  logic read_probe_mismatch_q;
  logic [7:0] read_probe_first_mismatch_q;
  ...
  assign read_probe_addr =
    PROBE_BASE_ADDR + {{(WB_ADDR_BITS - 8) {1'b0}}, read_probe_stream_index_q};
  assign read_probe_expected_data = {WB_SEL_BITS{read_probe_expected_byte}};
  assign read_probe_expected_byte =
    PROBE_BYTE_VALUE + read_probe_stream_index_q;
  assign read_probe_low_observed_byte = wb_data[7:0];
  ...
  always_ff @(posedge controller_clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      read_probe_stream_index_q <= 8'd0;
      read_probe_next_stream_index_q <= 8'd0;
      read_probe_observed_byte_q <= 8'd0;
      read_probe_write_drain_q <= 10'd0;
      read_probe_wait_cycles_q <= 32'd0;
      read_probe_match_q <= 1'b0;
      read_probe_mismatch_q <= 1'b0;
      read_probe_first_mismatch_q <= 8'd0;
      ...
      if (jtag_command_event) begin
        ...
        read_probe_err_seen_q <= 1'b0;
        read_probe_stall_seen_q <= 1'b0;
        read_probe_stream_index_q <= read_probe_next_stream_index_q;
        read_probe_next_stream_index_q <= read_probe_next_stream_index_q + 8'd1;
        read_probe_observed_byte_q <= 8'd0;
        read_probe_write_drain_q <= 10'd0;
        read_probe_wait_cycles_q <= 32'd0;
        read_probe_match_q <= 1'b0;
        ...
        READ_PROBE_ISSUE_WRITE: begin
          if (wb_stall) begin
            ...
            read_probe_stb_q <= 1'b0;
            read_probe_state_q <= wb_ack ? READ_PROBE_WAIT_WRITE_DRAIN : READ_PROBE_WAIT_WRITE_ACK;
          end
          if (wb_err) begin
        ...
        READ_PROBE_ISSUE_READ: begin
          ...
          if (wb_ack) begin
            ...
            read_probe_cyc_q <= 1'b0;
            read_probe_observed_byte_q <= read_probe_low_observed_byte;
            read_probe_match_q <= read_probe_low_observed_byte == read_probe_expected_byte;
            read_probe_mismatch_q <= read_probe_low_observed_byte != read_probe_expected_byte;
            read_probe_first_mismatch_q <=
              read_probe_low_observed_byte == read_probe_expected_byte ?
              8'hff : read_probe_stream_index_q;
            read_probe_done_q <= 1'b1;
            read_probe_state_q <= READ_PROBE_DONE;
          end
        end
  ...
  always_comb begin
    ...
    jtag_debug_payload[240 +: 32] =
      {read_probe_stream_index_q, read_probe_expected_byte,
       read_probe_observed_byte_q, PROBE_BYTE_VALUE};
    jtag_debug_payload[272 +: 32] =
      {read_probe_run_count_q[7:0], jtag_command_count[7:0],
       read_probe_stream_index_q[7:0], read_probe_expected_byte};
    ...
    jtag_debug_payload[416 +: 16] =
      {read_probe_first_mismatch_q, 4'd0, read_probe_state_q};
    jtag_debug_payload[432 +: 32] = clk50_count_q;
    jtag_debug_payload[464] = SYS_RSTN;
    jtag_debug_payload[465] = read_probe_match_q;
    jtag_debug_payload[466] = read_probe_mismatch_q;
    jtag_debug_payload[480 +: 32] =
      {8'd0, read_probe_stream_index_q, read_probe_expected_byte,
       read_probe_observed_byte_q};
  end
  ...
  ddr3_top #(
    ...
    .i_wb_addr(read_probe_addr),
    .i_wb_data(read_probe_expected_data),
    .i_wb_sel({WB_SEL_BITS{1'b1}}),
    .i_aux({3'd0, !read_probe_we_q}),
    .o_wb_stall(wb_stall),
    .o_wb_ack(wb_ack),
    .o_wb_err(wb_err),
    .o_wb_data(wb_data),
